...
class EpgmGveGraphCollectionFactory(var config: GradoopSparkConfig[L])(implicit session: SparkSession)
  ...
  override def init(graphHeads: Dataset[L#G], vertices: Dataset[L#V], edges: Dataset[L#E]): L#GC = {
    new GraphCollection[L](new EpgmGveLayout(graphHeads, vertices, edges), config) with GveGraphCollectionOperators[L]
  }
}
