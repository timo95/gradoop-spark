...
object CsvCollectionToIndexed extends EpgmConfigs {

  class Conf(arguments: Seq[String]) extends ScallopConf(arguments) {
    val input: ScallopOption[String] = opt[String](required = true,
      descr = "Input path for a csv collection")
    val output: ScallopOption[String] = opt[String](required = true,
      descr = "Output path for a indexed csv collection")
  ...
  def main(args: Array[String]): Unit = {
    ...
    sink.write(collection.asTfl(tflConfig), SaveMode.Overwrite)
  }
}
